object MainFrm: TMainFrm
  Left = 110
  Top = 115
  Caption = 'NBT Forge'
  ClientHeight = 508
  ClientWidth = 798
  Color = clBtnFace
  Font.Charset = DEFAULT_CHARSET
  Font.Color = clWindowText
  Font.Height = -11
  Font.Name = 'Tahoma'
  Font.Style = []
  GlassFrame.Enabled = True
  Menu = MainMenu
  OldCreateOrder = False
  Position = poDesigned
  WindowState = wsMaximized
  OnCloseQuery = FormCloseQuery
  OnCreate = FormCreate
  OnMouseWheel = FormMouseWheel
  OnResize = FormResize
  OnShow = FormShow
  DesignSize = (
    798
    508)
  PixelsPerInch = 96
  TextHeight = 13
  object CurrentBlockImage: TImage
    Left = 739
    Top = 23
    Width = 32
    Height = 32
    Anchors = [akTop, akRight]
    Picture.Data = {
      
      B046B967C70000000049454E44AE426082}
    ExplicitLeft = 743
  end
  object BlockListDropDown: TSpeedButton
    Left = 777
    Top = 8
    Width = 19
    Height = 47
    AllowAllUp = True
    Anchors = [akTop, akRight]
    GroupIndex = 1
    Caption = #9660
    Flat = True
    OnClick = BlockListDropDownClick
    ExplicitLeft = 781
  end
  object CurrentBlockName: TLabel
    Left = 741
    Top = 8
    Width = 28
    Height = 13
    Alignment = taRightJustify
    Anchors = [akTop, akRight]
    Caption = 'Stone'
    Transparent = True
    ExplicitLeft = 745
  end
  object Label1: TLabel
    Left = 24
    Top = 0
    Width = 26
    Height = 13
    Caption = 'Zoom'
  end
  object Label2: TLabel
    Left = 109
    Top = 0
    Width = 27
    Height = 13
    Caption = 'Layer'
  end
  object MapPanel: TPanel
    Left = 0
    Top = 61
    Width = 537
    Height = 427
    Anchors = [akLeft, akTop, akRight, akBottom]
    TabOrder = 0
    object Map: TImgView32
      Left = 1
      Top = 1
      Width = 535
      Height = 425
      Align = alClient
      Bitmap.ResamplerClassName = 'TNearestResampler'
      BitmapAlign = baCustom
      RepaintMode = rmOptimizer
      Scale = 1.000000000000000000
      ScaleMode = smScale
      ScrollBars.ShowHandleGrip = True
      ScrollBars.Style = rbsMac
      ScrollBars.Visibility = svAuto
      SizeGrip = sgNone
      OverSize = 0
      TabOrder = 0
      OnMouseDown = MapMouseDown
      OnMouseMove = MapMouseMove
      OnMouseUp = MapMouseUp
      OnMouseWheel = MapMouseWheel
      OnScroll = MapScroll
    end
  end
  object RightPanel: TPanel
    Left = 543
    Top = 61
    Width = 255
    Height = 427
    Anchors = [akTop, akRight, akBottom]
    BevelOuter = bvNone
    ParentShowHint = False
    ShowHint = True
    TabOrder = 1
    object RzSplitter1: TRzSplitter
      Left = 0
      Top = 0
      Width = 255
      Height = 427
      Orientation = orVertical
      Position = 280
      Percent = 66
      UsePercent = True
      RealTimeDrag = True
      HotSpotVisible = True
      HotSpotDirection = hsdMax
      SplitterStyle = ssGroupBar
      SplitterWidth = 7
      Align = alClient
      TabOrder = 0
      BarSize = (
        0
        280
        255
        287)
      UpperLeftControls = (
        ToolsPanel)
      LowerRightControls = (
        HistoryLogList)
      object ToolsPanel: TRzPageControl
        Left = 0
        Top = 0
        Width = 255
        Height = 280
        ActivePage = TabSheet1
        Align = alClient
        BoldCurrentTab = True
        UseColoredTabs = True
        MultiLine = True
        SortTabMenu = False
        TabIndex = 0
        TabOrder = 0
        Transparent = True
        FixedDimension = 19
        object TabSheet1: TRzTabSheet
          Caption = 'Brush'
          object ScrollBox1: TScrollBox
            Left = 0
            Top = 0
            Width = 251
            Height = 257
            HorzScrollBar.Smooth = True
            VertScrollBar.Smooth = True
            VertScrollBar.Tracking = True
            Align = alClient
            BevelEdges = []
            BevelInner = bvNone
            BevelOuter = bvNone
            BorderStyle = bsNone
            TabOrder = 0
          end
        end
        object TabSheet2: TRzTabSheet
          Caption = 'Line'
          ExplicitLeft = 0
          ExplicitTop = 0
          ExplicitWidth = 0
          ExplicitHeight = 0
        end
      end
      object HistoryLogList: TRzListBox
        Left = 0
        Top = 0
        Width = 255
        Height = 140
        Align = alClient
        BeepOnInvalidKey = False
        Color = clBtnFace
        ItemHeight = 13
        PopupMenu = HistoryLogMenu
        TabOrder = 0
      end
    end
  end
  object CheckBox1: TCheckBox
    Left = 440
    Top = 9
    Width = 97
    Height = 17
    Anchors = [akTop, akRight]
    Caption = 'Isometric View'
    Enabled = False
    TabOrder = 2
  end
  object RzStatusBar1: TRzStatusBar
    Left = 0
    Top = 489
    Width = 798
    Height = 19
    BorderInner = fsNone
    BorderOuter = fsNone
    BorderSides = [sdLeft, sdTop, sdRight, sdBottom]
    BorderWidth = 0
    TabOrder = 3
    object StatusMessage: TRzStatusPane
      Left = 0
      Top = 0
      Width = 543
      Height = 19
      Align = alClient
      ExplicitWidth = 539
    end
    object StatusProgress: TRzProgressStatus
      Left = 543
      Top = 0
      Width = 255
      Height = 19
      Align = alRight
      ParentShowHint = False
      PartsComplete = 0
      Percent = 0
      TotalParts = 0
      ExplicitLeft = 545
    end
  end
  object ZoomIn: TButton
    Left = 5
    Top = 16
    Width = 65
    Height = 18
    Caption = '+'
    Enabled = False
    TabOrder = 4
    OnClick = ZoomInClick
  end
  object ZoomOut: TButton
    Left = 5
    Top = 39
    Width = 65
    Height = 18
    Caption = '-'
    Enabled = False
    TabOrder = 5
    OnClick = ZoomOutClick
  end
  object LayerUp: TButton
    Left = 91
    Top = 16
    Width = 65
    Height = 18
    Caption = #9651
    Enabled = False
    TabOrder = 6
    OnClick = LayerUpClick
  end
  object LayerDown: TButton
    Left = 91
    Top = 39
    Width = 65
    Height = 18
    Caption = #9661
    Enabled = False
    Font.Charset = ANSI_CHARSET
    Font.Color = clWindowText
    Font.Height = -11
    Font.Name = 'Tahoma'
    Font.Style = []
    ParentFont = False
    TabOrder = 7
    OnClick = LayerDownClick
  end
  object ViewXY: TRadioButton
    Left = 178
    Top = 10
    Width = 49
    Height = 15
    Caption = 'X-Y'
    Checked = True
    TabOrder = 8
    TabStop = True
    OnClick = ViewXYClick
    OnMouseLeave = DeFocus
  end
  object ViewXZ: TRadioButton
    Left = 178
    Top = 26
    Width = 49
    Height = 15
    Caption = 'X-Z'
    TabOrder = 9
    OnClick = ViewXZClick
    OnMouseLeave = DeFocus
  end
  object ViewYZ: TRadioButton
    Left = 178
    Top = 42
    Width = 49
    Height = 15
    Caption = 'Y-Z'
    TabOrder = 10
    OnClick = ViewYZClick
    OnMouseLeave = DeFocus
  end
  object MainMenu: TMainMenu
    AutoLineReduction = maManual
    OwnerDraw = True
    Left = 432
    object mFile: TMenuItem
      Caption = '&File'
      object mNew: TMenuItem
        Caption = 'New...'
        Enabled = False
        ShortCut = 16462
        OnClick = mNewClick
      end
      object Open1: TMenuItem
        Caption = 'Open...'
        ShortCut = 16463
        OnClick = Open1Click
      end
      object N2: TMenuItem
        Caption = '-'
      end
      object mSave: TMenuItem
        Caption = 'Save...'
        Enabled = False
        ShortCut = 16467
        OnClick = mSaveClick
      end
      object mSaveAs: TMenuItem
        Caption = 'Save As...'
        Enabled = False
        OnClick = mSaveAsClick
      end
      object N1: TMenuItem
        Caption = '-'
      end
      object Import1: TMenuItem
        Caption = 'Import'
        Enabled = False
        object Heightmap1: TMenuItem
          Caption = 'Heightmap'
        end
      end
      object Export1: TMenuItem
        Caption = 'Export'
        Enabled = False
      end
      object N3: TMenuItem
        Caption = '-'
      end
      object mExit: TMenuItem
        Caption = 'Exit'
        ShortCut = 16465
        OnClick = mExitClick
      end
    end
    object Edit1: TMenuItem
      Caption = '&Edit'
      object mUndo: TMenuItem
        Caption = 'Undo'
        Enabled = False
        ShortCut = 16474
        OnClick = mUndoClick
        OnDrawItem = mUndoDrawItem
      end
    end
    object mView: TMenuItem
      Caption = '&View'
      Enabled = False
      object mGrid: TMenuItem
        AutoCheck = True
        Caption = 'Grid'
        ShortCut = 16455
        OnClick = mGridClick
      end
    end
    object mTools: TMenuItem
      Caption = '&Tools'
      Enabled = False
      object mCopyUp: TMenuItem
        Caption = 'Copy Layer Up'
        OnClick = mCopyUpClick
      end
      object mCopyDown: TMenuItem
        Caption = 'Copy Layer Down'
        OnClick = mCopyDownClick
      end
      object N4: TMenuItem
        Caption = '-'
      end
      object MapAttributions1: TMenuItem
        Caption = 'Map Attributes'
        OnClick = MapAttributions1Click
      end
      object BlockCount1: TMenuItem
        Caption = 'Block Counts'
        Enabled = False
      end
      object N5: TMenuItem
        Caption = '-'
      end
      object mFixChests: TMenuItem
        Caption = 'Fix Chests'
        OnClick = mFixChestsClick
      end
    end
    object Help1: TMenuItem
      Caption = '&Help'
      object mOfficialPost: TMenuItem
        Caption = 'Official Forum Post'
        OnClick = mOfficialPostClick
      end
      object About1: TMenuItem
        Caption = 'About'
      end
    end
  end
  object MenuController: TRzMenuController
    Left = 312
  end
  object OpenDialog: TOpenDialog
    Filter = 'Minecraft Level|*.mclevel'
    Options = [ofHideReadOnly, ofPathMustExist, ofNoNetworkButton, ofEnableSizing]
    Left = 352
  end
  object SaveDialog: TSaveDialog
    DefaultExt = '.mclevel'
    Filter = 'Minecraft Level|*.mclevel'
    Options = [ofOverwritePrompt, ofHideReadOnly, ofExtensionDifferent, ofShareAware, ofNoTestFileCreate, ofNoDereferenceLinks, ofEnableSizing]
    Left = 392
  end
  object HistoryLogMenu: TPopupMenu
    AutoLineReduction = maManual
    OwnerDraw = True
    OnPopup = HistoryLogMenuPopup
    Left = 472
    object mClear: TMenuItem
      Caption = 'Clear'
      OnClick = mClearClick
    end
  end
end
